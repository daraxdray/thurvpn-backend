    ...
    if (regions != null && Array.isArray(regions)) {
      ...
      if (msg != null) {
        return res.status(201).json({
          ...
          status: true,
          message:
            "Please ensure you provide all required fields for region: " + msg,
        });
      }
...
exports.getRegionByQuery = async (req, res) => {
  try {
    ...
    const region = findVpn.regions.find(
      (rg) =>
        rg.regionName.toLowerCase() == rn.toLowerCase() ||
        rg.slug.toLowerCase() == rn.toLowerCase() ||
        rg._id == rn
    );
    if (!region) {
      return res.status(400).json({
...
exports.updateRegion = async (req, res) => {
  try {
    ...
    const region = findVpn.regions.find((rg, index) => {
      regionIndex = index;
      return (
        rg.regionName.toLowerCase() == findBy.toLowerCase() ||
        rg.slug.toLowerCase() == findBy.toLowerCase() ||
        rg._id == findBy
      );
    });
    if (!region) {
...
exports.addRegions = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ data: [], status: true, message: "VPN server found" });
  } catch (error) {
    return failedResponseHandler(error, res);
  }
};
...
exports.deleteAllVpnWhere = async (req, res) => {
  try {
    const { deleteAccess, code } = req.body;
    if (deleteAccess == "damilola") {
      const deleted = await vpnModel.deleteMany({ countryCode: code });
      return res.status(200).json({
        data: deleted,
        status: true,
        message: "VPN cleared from documents",
